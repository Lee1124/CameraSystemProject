    ...
    <el-dialog
      ...
      <ul class="addNewHotelBox">
        ...
        <li>
          <label>地址：</label>
          <div class="commonBox">
            <commonInput :isReadonly="false" ref="childCommonInput_address" inputPlaceholderText="请输入酒店地址"></commonInput>
          </div>
        </li>
...
<script>
  ...
  let myMethods = {
    closeTip() {
      this.$parent.closeDialog('addHotel');
    },
    closeBefore() {
    ...
    addHotelRequest(data){
      let url = this.api + '/xlapi/CameraManage/CameraOrderInfoManage/CameraOrderInfo/AddCameraHotelInfo';
      this.$Axios({
        method: 'POST',
        url: url,
        data:data,
