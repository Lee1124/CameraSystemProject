...
<template>
  <div id="priceDetailsDiaLog">
          ...
          <div class="table">
            <el-table
              ...
              <el-table-column
                ...
                label="操作">
                <template slot-scope=scope>
                  <span style="cursor:pointer;" v-if="scope.row.satename!='已退回'" @click="look(scope.row,false)">查看</span>
                  <span style="cursor:pointer;color: #FF0000;" v-if="scope.row.satename=='已退回'" @click="look(scope.row,true)">编辑</span>
                </template>
              </el-table-column>
            </el-table>
          </div>
    ...
    <!--添加交款记录弹框-->
    <addPaymentRecordPage ref="addPaymentRecordPage" :showType="showType" type="list" :showAddPaymentRecordDiaLog="showAddPaymentRecordDiaLog"></addPaymentRecordPage>
  </div>
</template>
...
<script>
  ...
  let myMethods = {
    ...
    look(obj,type) {
      this.showType=type;
      this.showAddPaymentRecordDiaLog = true;
      this.$refs.addPaymentRecordPage.getDefaultData(obj);
    },
    //交款金额表头类名
